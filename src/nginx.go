...
func ensureNginxProxyConfigs(log func(string)) error {
    ...
    if err := ensureNginxStreamModules(log); err != nil { return err }
    // Ensure stream include in main config
    if err := ensureNginxStreamInclude(); err != nil { return err }
    // Ensure dirs
...
func ensureNginxStreamModules(log func(string)) error {
    ...
    streamSo := filepath.Join(modDir, "ngx_stream_module.so")
    sslPrereadSo := filepath.Join(modDir, "ngx_stream_ssl_preread_module.so")
    haveStream := fileExists(streamSo)
    havePreread := fileExists(sslPrereadSo)
    if !haveStream || !havePreread {
        // Provide actionable error; installation flow should install libnginx-mod-stream
        missing := []string{}
        if !haveStream { missing = append(missing, streamSo) }
...
func writeFileIfChanged(path, content string, mode os.FileMode) error {
    if b, err := os.ReadFile(path); err == nil {
        if string(b) == content { return nil }
    } else {
